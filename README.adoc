...
Cuando necesitamos algo mucho más avanzado podemos utilizar archivos que finalicen con la extensión _*.wren*_. El más común es el llamado _main.wren_. Para ejecutar un archivo específico simplemente se debe utilizar `./wren <archivo.wren>`.

1. Creamos un nuevo archivo llamado `main.wren`
2. Escribimos `System.print("Plop!")` y Guardamos
3. Finalmente Ejecutamos `./wren main.wren`

Deberíamos ver un resultao similar a este:
